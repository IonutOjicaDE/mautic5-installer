...
COMPOSER_ALLOW_SUPERUSER=1 COMPOSER_PROCESS_TIMEOUT=10000 composer --working-dir="${MAUTIC_FOLDER}" require symfony/amazon-mailer --no-interaction > /dev/null 2>&1

if [[ $? -ne 0 ]]; then
  show_info ${ICON_ERR} "Error: Installation of symfony/amazon-mailer for Mautic${MAUTIC_COUNT} failed."
  show_info ${ICON_ERR} "Should the installation continue?"
  answer_yes_else_stop
fi
